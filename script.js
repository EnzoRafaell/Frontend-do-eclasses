...
async function atualizarDashboard() {
    ...
    const modalidades = await fetch(`${API}/modalidades`).then(r => r.json());
    const confrontos = await fetch(`${API}/confrontos`).then(r => r.json());
...
async function carregarJogadores() {
    ...
    dados.jogadores.forEach(jogador => {
        ...
        turma.textContent = `Turma: ${jogador.turma}`;

        const email = document.createElement("p");
        email.textContent = `Email: ${jogador.email}`;

        const time = document.createElement("p");
        time.textContent = `Time ID: ${jogador.timeId}`;
...
async function carregarTimes() {
    ...
    dados.times.forEach(time => {
        ...
        const turma = document.createElement("p");
        turma.textContent = `Turma: ${time.turma}`;

        const jogadores = document.createElement("p");
        jogadores.textContent = `Jogadores: ${time.jogadores}`;
...
async function carregarModalidades() {
    ...
    dados.modalidades.forEach(modalidade => {
        ...
        const titulo = document.createElement("h3");
        titulo.textContent = modalidade.nome;
...
async function carregarConfrontos() {
    ...
    dados.confrontos.forEach(confronto => {
        ...
        const modalidade = document.createElement("h3");
        modalidade.textContent = confronto.modalidade;
